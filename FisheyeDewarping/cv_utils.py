...
def copy_to(src_image, dst_image, position):
    ...
    # except ValueError:
    #     print('Cannot copy', shape, 'to', dst_image.shape)
...
class Warp:
    ...
    def __init__(self, width, height):
        ...
        self.CDdefault = np.array(
            [0, 0, 0, 0,
             width / 2, 0, height / 2, 0, width / 2, height / 2, 0.0, 0.0, 1.0,
             width / 2, 0, height / 2, 0, width / 2, height / 2, 0.0, 0.0, 1.0,
             0.0],
            np.float32
        )
        ...
        self.D = np.array(self.CDdefault[0:4], np.float32)  # Cam Distortion
        self.C = np.array(self.CDdefault[4:13], np.float32)  # Cam Matrix
        self.Cnew = np.array(self.CDdefault[13:22], np.float32)  # New Cam Matrix
    ...
    def dewarp(self, src_image, size, ratio=1.0):
...
class Rect:
    ...
    def locate_top(self, rect):
        self.left_top = (rect.left_top[X], rect.left_top[Y] - self.size[Y])
        self.right_bottom = (self.left_top[X] + self.size[X], self.left_top[Y])

    def locate_bottom(self, rect):
        self.left_top = (rect.left_top[X], rect.left_top[Y] + rect.size[Y])
        self.right_bottom = (self.left_top[X] + self.size[X], self.left_top[Y] + self.size[Y])
...
class Button(Rect):
    ...
    def draw(self, text=None, color=None):
        ...
        cv2.putText(
            ...
            (self.left_top[X] + 5, self.right_bottom[Y] - int(self.size[Y] / 2)),
    ...
    def press(self):
        self.pressed = True
        self.released = False
        self.draw(None, tuple(c / 2 for c in self.color))
    ...
    def contain(self, x, y):
        return super().contain(x, y)

    def print(self):
        print(self.left_top, self.right_bottom, self.text)

    def locate_left(self, button):
        super().locate_left(button)

    def locate_right(self, button):
        super().locate_right(button)
